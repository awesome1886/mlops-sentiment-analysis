...
def main():
    ...
    prop_result = prop_check.run(train_dataset=test_dataset, test_dataset=stream_dataset)
    
    # Extract the "Sentiment" drift score specifically
    sentiment_data = prop_result.value.get("Sentiment", 0.0)
    sentiment_drift = sentiment_data.get("drift_score", 0.0) if isinstance(sentiment_data, dict) else float(sentiment_data)
    
    print(f"Property Drift Score (Sentiment): {sentiment_drift:.4f} / Threshold: {property_drift_threshold}")
    ...
    pred_drift_score = pred_result.value if isinstance(pred_result.value, float) else pred_result.value.get('drift_score', 0.0)
    
    print(f"Prediction Drift Score: {pred_drift_score:.4f} / Threshold: {prediction_drift_threshold}")
